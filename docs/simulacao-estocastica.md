...
> perdeu em cada caso está na **Seção 9.5**. As nove coisas que **não** foram simplificadas, e o que
> quebra em cada uma se alguém as simplificar depois, estão na **Seção 4.14 (O PISO)**. A Seção 4.14
> é a única parte deste documento onde não há liberdade de projeto.
>
...
| 3 | os três desfechos, ensemble de sementes | estatística de canais, `N_final`, `t_50` | próximo |
...
- **~~Fusão sempre incrementa `E_int` por um valor não negativo.~~ RETRATADO em 2026-08-07 (b).**
  ...
  **A fusão pode injetar energia mecânica, e isso é uma propriedade do modelo, não um defeito de
  ...
  cinética suficiente para "pagar" esse poço, a diferença aparece como energia mecânica criada. É por
  isso que `INV-23(c)` existe — e é por isso que ele **não pode** ser uma cota apertada.
- **Fragmentação tem sinal indefinido em geral**, com o sinal corrigido:
  ...
  **Contido o runaway (Seção 4.6), o maior corpo fica em `~3 m_bar`, os pares ficam quase iguais,
  e a patologia desaparece sozinha** — razão pela qual este documento **não** acrescenta uma regra de
  razão de massa à fragmentação. Uma correção que se torna desnecessária ao corrigir a causa é
...
**O mapa simplificado BAIXA o teto de massa**, de `4.94` para `3.53 m_bar`. A conclusão qualitativa
não muda e fica mais folgada: com `p_fus` menor, o laço positivo é mais fraco e o teto desce. O
controle uniforme (`5.667`) é independente do mapa e permanece como está.
...
**A predição mais importante é a de robustez entre sementes, e ela ainda não foi feita.** Toda a
Seção 4.13.5 decide sobre **uma** execução. `INV-31` com `K_SEEDS = 4` é exatamente o que a testa:
se as `4` sementes concordarem que há runaway lento e não degenerado, a decisão de aceitar está
apoiada; se metade não produzir runaway, o comportamento é bimodal e a decisão precisa ser revista.
**[A]** — essa é a próxima medição, e ela vale mais que qualquer quarta rodada de modelagem.
...
**Enunciado (c), sinal.** `min_t E_int(t) >= -1e-2 * |E_0|` (afrouxado de `1e-3`; justificativa na
Seção 4.10).

**Se (a) falhar pelo teto.** Sinal trocado no acumulador, ou o termo mútuo `E_grav` ausente de
`E_int`. **Se (a) falhar pelo piso** (resíduo abaixo de `1e-8`), os termos de terceiro corpo estão
sendo computados contra a Seção 4.10.
...
**(C8) Predição de `N_final`, registrada antes da implementação.** Com `chi = 0.1`, massas iguais,
`Q = 0`, `dt = 5.0e-4`, `3 t_ff`: **`N_final ∈ [700, 800]`** de `1000`, e `t_50 > 3 t_ff`. **[A]** —
derivação e leitura das falhas na Seção 4.13.1. Este critério é **informativo, não bloqueante**: uma
predição fora da banda é uma investigação, não uma reprovação automática — mas cair fora dela sem
explicação é motivo para não seguir adiante.
...
| `TOL-EVENT-CONS` | `\|Δ(K+U+E_int)\| / \|E_0\|` através de um mapa de desfecho | `1e-5` (teto) e `1e-8` (piso) | `1e-4` | **afrouxado nesta revisão**: `E_int` é fechada ao nível do par e omite terceiros; resíduo medido `2.5e-6` **[M]**, margem `4x`. O piso impede o teste de virar vazio (`INV-23(a)`) |
...
| `TOL-EINT-DRIFT` | `\|ΔE_total/E_0\|` acumulada em `3 t_ff` com fusão | `<= 1e-2` | idem | **novo**: `~500` eventos x `2.5e-6` de resíduo de sinal único = `1.2e-3` **[M]**; margem `~8x` |
...
ENS_N_FINAL_PREDICTED  = (700, 900)      # [A] Sec. 4.13.6; medido 774
ENS_RUNAWAY_TFF        = (1.2, 2.0)      # [A] t_runaway / t_ff ; medido ~1.55
ENS_MAX_MASS_FRACTION  = (0.15, 0.60)    # [A] max_i m_i / M_real em 3 t_ff ; medido 0.3213
ENS_RUNAWAY_MIN_TFF    = 0.5             # [A] duracao minima da transicao; medido 1.43
ENS_EINT_MAG_RANGE     = (3.0, 40.0)     # [A] max_t |E_int|/|E_0| ; medido 10.83
...
**A IMPLEMENTAR — `resolve` e o que o acompanha. Esta é a assinatura obrigatória:**
...
**O que fica em aberto, e é a próxima medição.** Toda a Seção 4.13.5 decide sobre **uma** execução.
`INV-31` com `K_SEEDS = 4` testa se o runaway é robusto entre sementes. Se metade das sementes não
o produzir, o comportamento é bimodal e esta decisão precisa ser revista. **Essa medição vale mais
que qualquer quarta rodada de modelagem.**
...
19. **Não há crescimento descontrolado com os parâmetros padrão, e há teto fechado:**
    `m*/m_bar = (p_fus + k p_frag)/((1-k) p_frag)` com `k = 3/4 - a/2 = 0.70`, dando **`~3.5`** (mapa
    `(1/x,3,x)/Z`) e `5.67` (controle uniforme), isto é `~0.35%` da massa total. **A causa é a
    fragmentação, não o mapa de regime** — o teto existe até no controle uniforme —, e o parâmetro
    que o controla é `a = FRAG_F_MIN`.
...
37. **A Seção 4.12 errou por congelar `p_fus` e `p_frag` num `x` fixo**, quando `x` dependia da
    ...
    frente da massa. Corrigido `x`, o ponto fixo é genuíno e vale `~3.2 m_bar`. **Um ponto fixo
    calculado com coeficientes congelados só é um teto se os coeficientes não dependerem da variável
    de estado.**
38. **A expressão de fragmentação de `E_int` estava com o sinal invertido neste documento**, e foi
    ...
    problema escondido.**
39. **`INV-23(c)` passou de cota de SINAL para cota de MAGNITUDE**, `|E_int| <= 1 |E_0|`. A fusão
...
48. **A predição vigente descreve o runaway em vez de negá-lo** (Seção 4.13.6), e continua
    falsificável: `t_runaway ∈ [1.2, 2.0] t_ff`, duração da transição `> 0.5 t_ff`,
    `max m_i/M_real ∈ [0.15, 0.60]`. **A predição que mais importa ainda não foi feita:** toda esta
    decisão repousa sobre **uma** execução, e `INV-31` com `K_SEEDS = 4` testa se o runaway é robusto
    entre sementes. Se metade não o produzir, o comportamento é bimodal e a decisão precisa ser
    revista. **Essa medição vale mais que qualquer quarta rodada de modelagem.**
